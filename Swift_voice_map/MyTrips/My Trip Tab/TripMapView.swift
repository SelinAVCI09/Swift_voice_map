...
class SpeechRecognizer: ObservableObject {
    ...
    func startRecording() {
        ...
        audioEngine.inputNode.removeTap(onBus: 0) // Remove previous tap
...
struct TripMapView: View {
                    ...
                    HStack {
                        Button(action: {
                            ...
                            Image(systemName: speechRecognizer.audioEngine.isRunning ? "mic.fill" : "mic")
                                .foregroundColor(speechRecognizer.audioEngine.isRunning ? .red : .blue)
                                .padding()
                        }
                    }
                    if routeDisplaying {
                        HStack {
                            Button("Rotayı Temizle", systemImage: "xmark.circle") {
                                removeRoute()
                            }
                            .buttonStyle(.borderedProminent)
                            .fixedSize(horizontal: true, vertical: false)
                            Button("Adım Adım Rotayı Göster", systemImage: "location.north") {
                                showSteps.toggle()
                            }
                            .buttonStyle(.borderedProminent)
                            .fixedSize(horizontal: true, vertical: false)
                                        ...
                                        List {
                                            ...
                                            ForEach(1..<route.steps.count, id: \.self) { idx in
                                                VStack(alignment: .leading) {
                                                    Text("\(transportType == .automobile ? "Sürüş" : "Yürüyüş") \(MapManager.distance(meters: route.steps[idx].distance))")
                                                        .bold()
                                                        .accessibility(label: Text("\(transportType == .automobile ? "Sürüş" : "Yürüyüş") \(MapManager.distance(meters: route.steps[idx].distance))"))
                                                    Text(route.steps[idx].instructions)
                                                        .accessibility(label: Text(route.steps[idx].instructions))
                                                }
                                            }
                                        }
                                        .listStyle(.plain)
    ...
    func fetchRoute() async {
        if let userLocation = locationManager.userLocation, let selectedPlacemark = selectedPlacemark {
